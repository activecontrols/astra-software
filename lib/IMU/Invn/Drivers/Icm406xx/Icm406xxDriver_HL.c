...
int inv_icm406xx_init(struct inv_icm406xx *s, struct inv_icm406xx_serif *serif, void (*sensor_event_cb)(void *ctx, inv_icm406xx_sensor_event_t *event)) {
  int rc;
...
int inv_icm406xx_enable_accel_low_power_mode(struct inv_icm406xx *s) {
  ...
  if (accel_mode == ICM406XX_PWR_MGMT_0_ACCEL_MODE_OFF && gyro_mode == ICM406XX_PWR_MGMT_0_GYRO_MODE_OFF) {
...
int inv_icm406xx_configure_wom(struct inv_icm406xx *s, const uint8_t wom_x_th, const uint8_t wom_y_th, const uint8_t wom_z_th) {
  int status = 0;
...
int inv_icm406xx_enable_wom(struct inv_icm406xx *s) {
  ...
  data |= BIT_INT_SOURCE1_WOM_Z_INT1_EN | BIT_INT_SOURCE1_WOM_Y_INT1_EN | BIT_INT_SOURCE1_WOM_X_INT1_EN;
      ...
      */
      ...
      for (packet_count_i = 0; packet_count_i < packet_count; packet_count_i++) {
        ...
        if (header->bits.accel_bit) {
          ...
          /* new accel data */
          if ((event.accel[0] != INVALID_VALUE_FIFO) && (event.accel[1] != INVALID_VALUE_FIFO) && (event.accel[2] != INVALID_VALUE_FIFO))
        ...
        if (header->bits.gyro_bit) {
          ...
          */
          if ((s->gyro_start_time_us == UINT32_MAX) && (event.gyro[0] != INVALID_VALUE_FIFO) && (event.gyro[1] != INVALID_VALUE_FIFO) && (event.gyro[2] != INVALID_VALUE_FIFO))
        ...
        */
        if ((header->bits.gyro_bit) && (s->gyro_start_time_us != UINT32_MAX) && (event.gyro[0] != INVALID_VALUE_FIFO) && (event.gyro[1] != INVALID_VALUE_FIFO) &&
            (event.gyro[2] != INVALID_VALUE_FIFO) && (!header->bits.fsync_bit)) {
          uint32_t timestamp_us;

          /* Compute the delta time and handle a possible rollover */
          if ((uint32_t)(event.timestamp_fsync * 16 /* timestamp resolution is 16us */) < s->gyro_start_time_us)
...
int inv_icm406xx_set_accel_frequency(struct inv_icm406xx *s, const ICM406XX_ACCEL_CONFIG0_ODR_t frequency) {
  return inv_icm406xx_wr_accel_config0_odr(s, frequency);
}

int inv_icm406xx_set_gyro_frequency(struct inv_icm406xx *s, const ICM406XX_GYRO_CONFIG0_ODR_t frequency) {
...
int inv_icm406xx_reset_fifo(struct inv_icm406xx *s) {
  ...
  if (((gyr_mode != ICM406XX_PWR_MGMT_0_GYRO_MODE_OFF) || (acc_mode != ICM406XX_PWR_MGMT_0_ACCEL_MODE_OFF)) && (acc_mode != ICM406XX_PWR_MGMT_0_ACCEL_MODE_LP)) {
    ...
    */
    ...
    status |= inv_icm406xx_wr_fifo_config1_gyro_en(s, ICM406XX_FIFO_CONFIG1_GYRO_DIS);
    status |= inv_icm406xx_wr_fifo_config1_accel_en(s, ICM406XX_FIFO_CONFIG1_ACCEL_DIS);
    status |= inv_icm406xx_wr_signal_path_rst_fifo_flush(s, ICM406XX_SIGNAL_PATH_RESET_FIFO_FLUSH_EN);
    inv_icm406xx_sleep_us(100);
...
int inv_icm406xx_get_current_timestamp(struct inv_icm406xx *s, uint32_t *icm_time) {
   ...
   */
  status = inv_icm406xx_wr_signal_path_rst_tmst_strobe(s, ICM406XX_SIGNAL_PATH_RESET_TMST_STROBE_EN);
