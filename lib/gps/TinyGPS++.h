...
// custom definitions
struct PUBX_Position;

template <typename t> struct PUBX_Value {
  friend class TinyGPSPlus;
...
struct PUBX_Position {
  TinyGPSTime time;
  TinyGPSLocation loc;
  PUBX_Value<float> altRef;       // Altitude above user datum ellipsoid (meters)
  PUBX_Value<Nav_Status> navStat; // Navigation Status
  PUBX_Value<float> hAcc;         // Horizontal accuracy estimate (meters)
  PUBX_Value<float> vAcc;         // Vertical accuracy estimate (meters)
  PUBX_Value<float> SOG;          // Speed over ground (km/h)
  PUBX_Value<float> COG;          // Course over ground (deg)
  PUBX_Value<float> vVel;         // Vertical velocity (positive downwards) (meters/second)
  // I have no idea how diffAge is supposed to be formatted so I am skipping it
  PUBX_Value<float> HDOP; // HDOP, Horizontal Dilution of Precision
  PUBX_Value<float> VDOP; // VDOP, Vertical Dilution of Precision
  PUBX_Value<float> TDOP; // TDOP, Time Dilution of Precision
  PUBX_Value<int> numSvs; // Number of satellites used in the navigation solution
};

// end custom definitions

// idk how to use and cba to figure it out as their docs are down
class TinyGPSCustom {
public:
  TinyGPSCustom() {};
...
public:
  ...
  // custom field (contains information from the most recent NMEA-PUBX-POSITION sentence)
  PUBX_Position pubx_position;
...
private:
  enum { GPS_SENTENCE_GGA, GPS_SENTENCE_RMC, GPS_SENTENCE_PUBX_POSITION, GPS_SENTENCE_PUBX_OTHER, GPS_SENTENCE_OTHER };
